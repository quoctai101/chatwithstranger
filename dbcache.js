...
var Partner1 = function(id2, genderok, preferok, starttime) {
	this.partner = ''+id2;
	this.id2 = id2;
	this.genderok = genderok;
	this.preferok = preferok;
	this.starttime = starttime;
}

var Partner2 = function(id1, genderok, preferok, starttime) {
	this.partner = ''+id1;
	this.id1 = id1;
	this.genderok = genderok;
	this.preferok = preferok;
...
var wr_write = function(id, gender, prefer, time) {
	waitroom.put(''+id, {gender:gender, prefer:prefer, time:time});
}
...
var wr_read = function(callback) {
	var ids = []; var genders = []; var prefers =[]; var time = [];
	...
	for (var i=0 ; i<db.length ; i++) {
		...
		prefers.push(temp.prefer);
		time.push(temp.time);
	}
	callback(ids, genders, prefers, time);
}

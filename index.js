...
function findPair(id, mygender, isPreferedGender) {
  if (mygender == 0) {
	  sendTextMessage(id, la.BATDAU_WARN_GENDER);
	  sendButtonMsg(sender, la.HUONG_DAN, true, true);
  }
  else {
	// lấy list waitroom trước
	tools.getListWaitRoom(sqlconn, function(list, genderlist, preferlist) {
		processWaitRoom(0);
		function processWaitRoom(i) {
			if (i >= list.length) {
				// nếu ko có ai phù hợp để ghép đôi, xin mời vào ngồi chờ
				tools.writeToWaitRoom(sqlconn, id, mygender, isPreferedGender);
				sendTextMessage(id, la.BATDAU_OKAY);
				return;
			}
			var target = list[i];
			var target_gender = genderlist[i];
			var target_prefer = preferlist[i];
